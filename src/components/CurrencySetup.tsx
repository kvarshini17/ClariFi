  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden transition-colors">
      ...
      <motion.div 
        ...
        className="max-w-xl w-full bg-white/40 dark:bg-zinc-900/40 backdrop-blur-3xl border border-border rounded-[48px] p-8 sm:p-16 shadow-[0_40px_100px_rgba(0,0,0,0.1)] dark:shadow-[0_40px_100px_rgba(0,0,0,0.5)] relative z-10 transition-colors"
      >
        <div className="text-center space-y-6 mb-12">
          <motion.div 
            ...
            className="w-20 h-20 bg-accent/10 rounded-[24px] flex items-center justify-center mx-auto border border-accent/20 shadow-2xl shadow-emerald-500/10"
          >
            <Globe className="text-emerald-400" size={36} />
          </motion.div>
          <div className="space-y-2">
            <h1 className="text-5xl font-black text-primary tracking-tighter leading-tight">Global Context</h1>
        ...
        <div className="space-y-8">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full bg-zinc-50 dark:bg-white/5 border border-border rounded-2xl py-5 pl-14 pr-6 text-primary placeholder:text-zinc-400 dark:placeholder:text-zinc-700 focus:ring-2 focus:ring-emerald-500 outline-none transition-all font-bold"
            />
          </div>
            ...
            {filtered.map((country) => (
              <button
                ...
                className={`flex items-center justify-between p-5 rounded-2xl border transition-all group ${
                  selected?.code === country.code 
                    ? 'bg-accent/20 border-accent text-primary shadow-lg shadow-emerald-500/10' 
              ...
              >
                <div className="text-left">
                  <p className={`text-sm font-black tracking-tight transition-colors ${selected?.code === country.code ? 'text-primary' : 'text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white'}`}>{country.name}</p>
                  <p className="text-xs font-black uppercase tracking-[0.2em] opacity-60 mt-1 text-secondary">{country.currency.code} • {country.currency.symbol}</p>
                </div>
                {selected?.code === country.code && (
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-accent rounded-full p-1">
          ...
          <button
            ...
            className="w-full group relative px-8 py-6 bg-accent disabled:bg-zinc-800 disabled:text-zinc-600 text-zinc-950 rounded-[24px] font-black text-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 overflow-hidden shadow-2xl shadow-emerald-500/20"
          >
