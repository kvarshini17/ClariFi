      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 mb-2">
            <span className="text-[13px] font-black text-zinc-500 dark:text-secondary uppercase tracking-widest">Milestones</span>
          </div>
          <h3 className="text-[27px] font-black text-primary tracking-tight">Financial Goals</h3>
          <p className="text-secondary text-[15px] font-bold uppercase tracking-widest">Turn your dreams into reality</p>
        </div>
        <button 
          onClick={onAddClick}
          style={{ height: '45px', width: '160.575px', lineHeight: '23px' }}
          className="bg-blue-500 text-white rounded-2xl shadow-lg shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all font-black text-[14px] flex items-center justify-center gap-2 px-4"
        >
          <Plus size={18} />
          Add Goal
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            return (
              ...
              >
                <div className={`absolute top-0 right-0 w-32 h-32 blur-3xl -mr-16 -mt-16 transition-colors ${
                  isCompleted ? 'bg-accent/10' : 'bg-blue-500/5 group-hover:bg-blue-500/10'
                }`} />
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-black text-primary text-[19px]">{goal.title}</h4>
                        {isCompleted && (
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                ...
                <div className="space-y-4 relative z-10">
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      <p className="text-[15px] font-bold text-zinc-500 dark:text-secondary uppercase tracking-widest">Progress</p>
                      <p className="text-xl font-black text-primary">
                        {currencySymbol}{goal.currentAmount.toLocaleString()} 
                        <span className="text-zinc-400 text-[13px] font-medium ml-1">/ {currencySymbol}{goal.targetAmount.toLocaleString()}</span>
                      </p>
                    </div>
                    <div className="text-right">
                      <p className={`text-2xl font-black ${isCompleted ? 'text-accent' : 'text-blue-500'}`}>
                        {Math.round(progress)}%
                      </p>
                    </div>
                  </div>

                  <div className="h-3 bg-zinc-100 dark:bg-white/5 rounded-full overflow-hidden border border-border">
                    <motion.div 
                      initial={{ width: 0 }}
                    ...
                    <div className="flex flex-col xs:flex-row gap-2 mt-4">
                      <div className="relative flex-1">
                        ...
                        <input 
                          ...
                          className="w-full bg-zinc-50 dark:bg-white/5 border border-border rounded-xl pl-8 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-bold"
                        />
                      </div>
                      <button 
                        onClick={() => handleContribute(goal)}
                        disabled={isSubmitting === goal.id}
                        className="px-4 py-2 bg-card text-primary rounded-xl text-xs font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg disabled:opacity-50 disabled:scale-100"
                      >
                  ...
                  {isCompleted && (
                    <motion.div 
                      ...
                      className="bg-accent/10 border border-accent/20 rounded-2xl p-3 flex items-center gap-3"
                    >
                      <div className="bg-accent text-white p-1.5 rounded-lg">
                        <Sparkles size={14} />
                      </div>
                      <p className="text-xs font-black text-accent dark:text-accent uppercase tracking-widest">Goal Achieved! You're amazing.</p>
                    </motion.div>
                  )}
        ...
        {goals.length === 0 && (
          <div className="col-span-full py-16 text-center space-y-4 glass-card rounded-[40px] border-dashed">
            ...
            <div className="space-y-1">
              <p className="text-primary font-black text-xl">No goals set yet</p>
              <p className="text-zinc-500 text-sm">Start tracking your financial milestones today.</p>
            </div>
            <button 
              onClick={onAddClick}
              className="px-6 py-3 bg-card text-primary rounded-2xl font-bold text-sm hover:scale-105 transition-all"
            >
