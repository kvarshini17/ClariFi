...
export default function LegalContent({ type, onBack }: LegalContentProps) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white dark:bg-white/5 backdrop-blur-xl p-8 sm:p-12 rounded-[40px] border border-border shadow-2xl space-y-12 transition-colors duration-300">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-zinc-50 dark:bg-white/5 rounded-2xl flex items-center justify-center border border-border shadow-xl">
            {content.icon}
          </div>
          <div className="space-y-1">
            <h3 className="text-4xl font-black text-primary tracking-tight">{content.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {content.sections.map((section, index) => (
            <div key={index} className="space-y-4 p-6 bg-zinc-50 dark:bg-white/5 rounded-3xl border border-zinc-200 dark:border-white/5 hover:border-accent/30 transition-colors group">
        ...
        <div className="pt-8 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <button 
            onClick={onBack}
            className="px-8 py-4 bg-card text-primary rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-xl"
          >
            I Understand
          </button>
        </div>
